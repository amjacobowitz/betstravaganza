...
export interface OddsApiOddsGame {
  id: string
  sport_key: string
  commence_time: string
  home_team: string
...
export async function fetchOdds(sportKey: string, markets: string[]): Promise<OddsApiOddsGame[]> {
  const key = process.env.ODDS_API_KEY
  if (!key) throw new Error('ODDS_API_KEY not configured')
  const res = await fetch(
    `${BASE}/sports/${sportKey}/odds?apiKey=${key}&regions=us&markets=${markets.join(',')}&oddsFormat=american`,
    { cache: 'no-store' },
  )
  if (!res.ok) {
    const text = await res.text().catch(() => '')
